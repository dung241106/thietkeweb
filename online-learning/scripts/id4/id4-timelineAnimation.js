...
class UbuntuTimeline {
    constructor() {
      this.config = {
        ...
        colors: {
          ...
          background: '#F5F6F5' // --ubuntu-light
        }
      };
      this.init();
    }
    ...
    centerVerticalLine() {
      ...
      line.style.cssText = `
        ...
        bottom: 0;
        width: 4px;
        background: linear-gradient(to bottom, ${this.config.colors.primary}, ${this.config.colors.secondary});
        z-index: 1;
      `;
  
      if (this.config.enableGlow) {
        line.style.boxShadow = `0 0 8px ${this.config.colors.primary}, 0 0 16px rgba(233, 84, 32, 0.3)`;
      }
    ...
    positionDot(dot, item, line, container) {
      ...
      dot.style.cssText = `
        ...
        z-index: 3;
        box-shadow: 0 0 0 3px ${this.config.colors.background}, 0 0 0 6px rgba(233, 84, 32, 0.3);
      `;
    ...
    setupAnimations() {
      ...
      document.querySelectorAll(this.config.timelineItem).forEach(item => {
        ...
        item.addEventListener('mouseenter', () => {
          gsap.to(content, {
            y: -3,
            boxShadow: '0 10px 25px rgba(233, 84, 32, 0.3)', // --ubuntu-orange
            duration: 0.25
          });
        });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    if (typeof gsap !== 'undefined') {
      ...
      script.onload = () => new UbuntuTimeline();
